...
public:
  ...
  explicit TwoPartDateUTC(const ymd_date &ymd) noexcept
      : _mjd(modified_julian_day(ymd.yr(), ymd.mn(), ymd.dm())
                 .as_underlying_type()),
        _fsec(0e0) {}
  ...
  [[nodiscard]] TwoPartDateUTC
  add_seconds(FractionalSeconds fsec) const noexcept {
...
public:
   ...
   */
  static TwoPartDate from_cnes_jd(double cnes_jd) noexcept {
    double ipart;
    const double fpart = std::modf(cnes_jd, &ipart);
    return TwoPartDate((int)ipart + (int)MJD_MINUS_CNESJD,
                       FractionalSeconds(fpart * 86400e0));
  }
   ...
   */
  TwoPartDate tt2gps() const noexcept { return tt2tai().tai2gps(); }
  ...
  TwoPartDate gps2tt() const noexcept { return gps2tai().tai2tt(); }
  ...
  TwoPartDate tt2ut1(FDOUBLE dut1) const noexcept {
    ...
    // return ut1;
    const auto utc = this->tt2utc();
    return TwoPartDate(utc.imjd(), utc.seconds())
        .add_seconds(FractionalSeconds(dut1));
  }
