...
double
to_sec(ngpt::hours h, ngpt::minutes m, ngpt::seconds s, double fs)
{
    long sec = 
        h.as_underlying_type() * 3600 +
        m.as_underlying_type() * 60   +
...
int main()
{
    ...
    std::cout<<"\n\tSeconds = "<< to_sec(std::get<0>(hmsf), std::get<1>(hmsf), std::get<2>(hmsf), 0e0);
    ...
    std::cout<<"\n\tSeconds = "<< to_sec(std::get<0>(hmsf), std::get<1>(hmsf),
        std::get<2>(hmsf), std::get<3>(hmsf)/milliseconds::sec_factor<double>());
    ...
    std::cout<<"\n\tSeconds = "<< to_sec(std::get<0>(hmsf), std::get<1>(hmsf),
        std::get<2>(hmsf), std::get<3>(hmsf)/microseconds::sec_factor<double>());
