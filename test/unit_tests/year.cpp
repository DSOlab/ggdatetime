...
int main() {
  ...
  assert(y1 <= year(2024));
  assert(y1 <= year(2025));
  assert(y1 + year(1) == year(2025));
  assert(y1 - year(1) == year(2023));
  assert((y1 += year(1)) == year(2025));
  assert((y1 -= year(1)) == year(2024));
  // only allowed with ALLOW_DT_INTEGRAL_MATH
  // assert((y1 += 1) == year(2025));
  // assert((y1 -= 1) == year(2024));
  assert((y1++) == year(2024));
  assert((++y1) == year(2026));
  assert((y1--) == year(2026));
  assert((--y1) == year(2024));

  /* leap years in range [1804, 2400] */
  const std::vector<int> leap = {
